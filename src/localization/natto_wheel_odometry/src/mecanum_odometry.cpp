...
#include "natto_wheel_odometry/mecanum_odometry.hpp"

namespace mecanum_odometry {

mecanum_odometry::mecanum_odometry (const rclcpp::NodeOptions &node_options) : Node ("mecanum_odometry", node_options) {
    ...
    RCLCPP_INFO (this->get_logger (), "mecanum_odometry node has been initialized.");
...
void mecanum_odometry::mecanum_callback (const natto_msgs::msg::Mecanum::SharedPtr msg) {
    if (msg->wheel_speed.size () != num_wheels_) {
        RCLCPP_ERROR (this->get_logger (), "Received mecanum message with incorrect number of wheels.");
        return;
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE (mecanum_odometry::mecanum_odometry)
