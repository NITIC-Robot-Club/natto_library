...
joy_to_twist::joy_to_twist (const rclcpp::NodeOptions &node_options) : Node ("joy_to_twist", node_options) {
    ...
    slow_max_xy_speed_m_s_ = this->declare_parameter<double> ("slow_max_xy_speed_m_s", 0.5);
    fast_max_xy_speed_m_s_ = this->declare_parameter<double> ("fast_max_xy_speed_m_s", 2.0);
    ...
    RCLCPP_INFO (this->get_logger (), "fast_max_xy_speed_m_s: %.2f", fast_max_xy_speed_m_s_);
...
void joy_to_twist::joy_callback (const sensor_msgs::msg::Joy::SharedPtr msg) {
    const bool release_button_pressed = left_stick_release_button_ >= 0 &&
        static_cast<size_t> (left_stick_release_button_) < msg->buttons.size () &&
        msg->buttons[static_cast<size_t> (left_stick_release_button_)] != 0;
    const bool release_button_positive_edge = release_button_pressed && !last_release_button_pressed_;
    const bool left_stick_is_moving =
        left_stick_x_axis_ >= 0 && left_stick_y_axis_ >= 0 &&
        static_cast<size_t> (left_stick_x_axis_) < msg->axes.size () &&
        static_cast<size_t> (left_stick_y_axis_) < msg->axes.size () &&
        (std::abs (msg->axes[static_cast<size_t> (left_stick_x_axis_)]) > left_stick_motion_threshold_ ||
         std::abs (msg->axes[static_cast<size_t> (left_stick_y_axis_)]) > left_stick_motion_threshold_);
    const bool left_stick_is_stopped =
        left_stick_x_axis_ >= 0 && left_stick_y_axis_ >= 0 &&
        static_cast<size_t> (left_stick_x_axis_) < msg->axes.size () &&
        static_cast<size_t> (left_stick_y_axis_) < msg->axes.size () &&
        std::abs (msg->axes[static_cast<size_t> (left_stick_x_axis_)]) <= left_stick_motion_threshold_ &&
    ...
    const double xy_speed = speed_limit_release_active_ ? fast_max_xy_speed_m_s_ : slow_max_xy_speed_m_s_;
