...
: Node("canable", node_options) {
    ...
    canable_sub_ = this->create_subscription<natto_msgs::msg::Can>(
        "/can/transmit", 10,
        [this](const natto_msgs::msg::Can::SharedPtr msg) {
...
void canable::write_can_socket(const natto_msgs::msg::Can &msg) {
    ...
    while (write(can_socket_, &frame, sizeof(frame)) < 0) {
        if (!retry_write_can || attempt >= max_retry_write_count) {
            RCLCPP_ERROR(get_logger(), "CAN write failed after %d retries on %s",
                         attempt, can_interface.c_str());
            init_can_socket();
            return;
        }
        attempt++;
        RCLCPP_WARN(get_logger(), "CAN write failed, retry %d/%d on %s",
                    attempt, max_retry_write_count, can_interface.c_str());
