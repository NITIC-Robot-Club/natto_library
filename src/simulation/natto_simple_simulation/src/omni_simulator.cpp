...
omni_simulator::omni_simulator (const rclcpp::NodeOptions &node_options) : Node ("omni_simulator", node_options) {
    ...
    RCLCPP_INFO (this->get_logger (), "Wheel speed gain P: %.2f, D: %.2f", wheel_speed_gain_p_, wheel_speed_gain_d_);
...
void omni_simulator::timer_callback () {
    ...
    for (int i = 0; i < received_commands.size (); i++) {
        for (int j = 0; j < num_wheels_; j++) {
            if (received_commands[i].wheel_speed.size () != num_wheels_) {
    ...
    omni_result_publisher_->publish (result);
