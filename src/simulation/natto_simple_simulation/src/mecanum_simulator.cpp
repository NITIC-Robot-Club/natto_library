...
#include "natto_simple_simulation/mecanum_simulator.hpp"
...
mecanum_simulator::mecanum_simulator (const rclcpp::NodeOptions &node_options) : Node ("mecanum_simulator", node_options) {
    ...
    RCLCPP_INFO (this->get_logger (), "mecanum_simulator node has been initialized.");
    ...
    timer_ = this->create_wall_timer (std::chrono::duration<double> (1.0 / frequency_), std::bind (&mecanum_simulator::timer_callback, this));
}

void mecanum_simulator::mecanum_command_callback (const natto_msgs::msg::Mecanum::SharedPtr msg) {
    received_commands_.push_back (*msg);
}

void mecanum_simulator::timer_callback () {
    if (received_commands_.empty ()) {
        received_commands_.push_back (result_);
    }
    natto_msgs::msg::Mecanum command_sum;
    ...
    mecanum_result_publisher_->publish (result_);
...
RCLCPP_COMPONENTS_REGISTER_NODE (mecanum_simulator::mecanum_simulator)
