...
| steering_stop_mode | string | "turn" | swerve停止時のステアリング制御（`turn`, `lock`, `hold`） |

`steering_stop_mode` の意味は次の通りです。
- `turn`: 現在の挙動。停止時に各ホイールを旋回向けの角度へ向ける
- `lock`: 現在のステアリング角度から 90 度ずらして、停止時の姿勢を固定しやすくする
- `hold`: 直前の指令で使っていたステアリング方向をそのまま保持する
