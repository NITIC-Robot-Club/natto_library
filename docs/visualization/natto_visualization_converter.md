...
# swerve_visualizer
swerve_visualizer ノードは、`joint_states` を受信し、swerve シャーシの各ホイールの向きと速度をベクトルとして可視化します。launch ではこの購読先を `command_joint_states` や `joint_states` に remap して使います。

## 機能
- `chassis_calculator` と同じパラメータ名で swerve 用の設定を受け取る
- `command_joint_states` からは、ホイール速度矢印とステア速度の円弧を `alpha 0.5` で描画し、launch ではホイール速度矢印を青で表示する
- `/joint_states` からは、同じホイール速度矢印とステア速度の円弧を `alpha 1.0` で描画する
...
- RViz では `/visualization/swerve_command` や `/visualization/swerve_states` の `MarkerArray` ディスプレイで確認できる
...
| steering_color_r | double | 0.0 | 円弧とステア矢印の赤成分 |
| steering_color_g | double | 0.0 | 円弧とステア矢印の緑成分 |
| steering_color_b | double | 1.0 | 円弧とステア矢印の青成分 |
...
| トピック名 | メッセージ型 | 説明 |
| - | - | - |
| joint_states | sensor_msgs/msg/JointState | `launch` で `command_joint_states` や `joint_states` に remap して使う |
